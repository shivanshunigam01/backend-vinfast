...
const {
  computeSlotsForBranchDate: buildSlots,
} = require("../utils/tdSlotAvailability");
const {
  ensureTdBranch,
  ensureTdSlotConfig,
  ensureTdFleet,
...
function formatSlotConfig(doc) {
  ...
  const branch =
    plain.branchId && typeof plain.branchId === "object"
      ? plain.branchId
      : null;
  return {
    _id: plain._id,
...
exports.listConfigs = asyncHandler(async (req, res) => {
  const docs = await TDSlotConfig.find({})
...
exports.availableSlots = asyncHandler(async (req, res) => {
  ...
  if (!branchId || !date)
    throw new ApiError(400, "branchId and date are required");

  const config = await ensureBranchSlotData(branchId);
  const { slots, fleetCapacity } = await computeSlotsForBranchDate(
    branchId,
    String(date),
    {
...
exports.saveDateOverrides = asyncHandler(async (req, res) => {
  ...
  if (!config)
    throw new ApiError(404, "Slot configuration not found for branch");
  ...
  return successResponse(
    res,
    formatSlotConfig(config),
    "Daily slot overrides saved",
  );
});
